...
M = SimpleNamespace(
    ...
    architecture = 'cognet_v16',  # used to load architecture and name of top-level results directory
    architecture_args = dict(cycles=4),  # key word args when initializing the model
    ...
    identifier = None,  # name of second-level results directory
    model_dir = op.expanduser(f'~/david/models/cognet/v16'), #None, # manual override for results directory
...
D = SimpleNamespace(
    ...
    transform_type = 'default',  # 'contrastive' or 'default'
    cutmix = False,
    cutmix_args = dict(
        prob=1.0,
        alpha=1,
...
O = SimpleNamespace(
    ...
    optimizer_args = dict(
        lr=1e-3,  # set float or one of the following strings: 'LRfinder', 'batch_linear', 'batch_nonlinear'
        weight_decay=1e-2,
        #momentum=.9,
    ),
    overwrite_optimizer=False,
    scheduler = 'ReduceLROnPlateau', #StepLR',  # any attribute of torch.optim.lr_scheduler
    scheduler_args = dict(
        #gamma=0.1, step_size = 100,  # StepLR
        patience = 8, factor = .1,  # ReduceLROnPlateau
    ),
