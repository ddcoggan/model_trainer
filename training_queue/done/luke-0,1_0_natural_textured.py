...
M = SimpleNamespace(
    ...
    identifier = 'hd-2_hw-3_V1f-128_occ-nat_xform-cont',  # used to name model directory, required
    save_interval = 4,  # preserve params at every n epochs
    #transfer = True,
    #transfer_dir = 'transfer_unocc',
    #freeze_layers = ['encoder'],  # freeze params up to this index (for transfer learning) cornet_s = 84
    #reinitialize_layers = ['head'],
    # return_model = False,  # return model object to environment after training
    #model_dir = 'models/head-depth-2_head-width-3_V1-feat-128_occ-nat_transform-cont',   # starting params model and epoch (e.g. when starting transfer learning)
...
if __name__ == "__main__":
    ...
    if not hasattr(CFG.M, 'model_dir'):
        CFG.M.model_dir = op.expanduser(f'~/david/projects/model_trainer/models/{CFG.M.model_name}/{CFG.M.identifier}')
