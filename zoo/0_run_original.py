...
# python run_reduce.py train --model RTREDUCEITACT --workers 20 --ngpus 1 --step_size 10 --epochs 25 --lr .1 --data_path '/media/dave/8e469ae6-e312-48a9-84ed-9bd61fb910db/home/dave/Datasets/imagenet1000' --output_path './cornet/'
...
parser.add_argument('-o', '--output_path', default='./cornet/cornet_rt_SFHighToClear_4ppdcutoff1cpd4TimeStep_16cate/',
...
def set_gpus(n=1):
    ...
    gpus = gpus[gpus['memory.total [MiB]'] > 20000]  # only above 10 GB
...
def get_model(pretrained=False):
    ...
    if FLAGS.model.lower() == 'rtnormwithinrf':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times)
    elif FLAGS.model.lower() == 'rtsflowtohigh':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff,timeStepCutoff = FLAGS.timeStepCutoff)
    elif FLAGS.model.lower() == 'rtsfhightolow':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff,timeStepCutoff = FLAGS.timeStepCutoff)
    elif FLAGS.model.lower() == 'rtsflowtoclear':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff,timeStepCutoff = FLAGS.timeStepCutoff)
    elif FLAGS.model.lower() == 'rtsfhightoclear':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff,timeStepCutoff = FLAGS.timeStepCutoff)
    elif FLAGS.model.lower() == 'rtsfhigh':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff)
    elif FLAGS.model.lower() == 'rtsflow':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times,
                      ppd = FLAGS.ppd, cpdCutOff = FLAGS.cpdCutOff)
    elif FLAGS.model.lower() == 'st':
        model = model(pretrained=pretrained, map_location=map_location, times=FLAGS.times)
    else:
...
def train(restore_path = None,#FLAGS.output_path+'latest_checkpoint.pth.tar',#None,  # useful when you want to restart training
          save_train_epochs=.1,  # how often save output during training
          save_val_epochs=.5,  # how often save output during validation
          save_model_epochs=25,  # how often save model weights
          save_model_secs=10 #* 10  # how often save model (in sec)
          ):
    ...
    if start_epoch == 0:
        for m in model.module.V1.modules():
            ...
            # to add it during the training of this network
                ...
                m.bias.data.zero_()
        for m in model.module.V2.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
            # nn.Linear is missing here because I originally forgot
            # to add it during the training of this network
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
        for m in model.module.V4.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
            # nn.Linear is missing here because I originally forgot
            # to add it during the training of this network
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
        for m in model.module.IT.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
            # nn.Linear is missing here because I originally forgot
    ...
    for epoch in tqdm.trange(start_epoch, FLAGS.epochs + 1, initial=start_epoch, desc='epoch'):
        ...
        train_res = {'loss': 0, 'top1': 0, 'top5': 0}
        with tqdm.tqdm(trainer.data_loader) as tepoch:
            for step, data in enumerate(tepoch):
                tepoch.set_description(trainer.name + f" Epoch {epoch}")
        #     for step, data in enumerate(tqdm.tqdm(trainer.data_loader, desc=trainer.name)):
                ...
                if epoch < FLAGS.epochs:
                    ...
                    train_res['top1'] += record['top1Count']
                    train_res['top5'] += record['top5Count']
                    train_res['loss'] += record['loss']
                ...
                tepoch.set_postfix(loss = record['loss'],top1 = record['top1'],top5 = record['top5'])
                data_load_start = time.time()
        res = {}
        res[validator.name] = validator()
        res[trainer.name] = record
        stat_file = open(os.path.join(FLAGS.output_path, 'training_stats.txt'), 'a+')
        stat_str = '[Training] Epoch ({}) LR ({:.4f}) Loss ({:.6f}) Accuracy1 ({:.4f}) Accuracy5 ({:.4f})'. \
            format(epoch, res[trainer.name]['learning_rate'], train_res['loss']/(step+1),
                   train_res['top1']/len(trainer.data_loader.dataset.samples), train_res['top5']/len(trainer.data_loader.dataset.samples))
        stat_file.write(stat_str + '\n')
        stat_file.close()
        stat_file = open(os.path.join(FLAGS.output_path, 'training_stats.txt'), 'a+')
        stat_str = '[Validation] Epoch ({}) Loss ({:.6f}) Accuracy1 ({:.4f}) Accuracy5 ({:.4f})'. \
            format(epoch, res[validator.name]['loss'],
                   res[validator.name]['top1'], res[validator.name]['top5'])
        stat_file.write(stat_str + '\n')
        stat_file.close()
...
# class totalActivation():
#     def forward(self,input):
#         return torch.sum(torch.abs(input.Flatten()))

# def totalActivation(input):
#     return torch.mean(torch.abs(input.view(-1)))
...
class ImageNetTrain(object):
    ...
    def __init__(self, model):
        ...
        # self.loss2 = totalActivation()
        if FLAGS.ngpus > 0:
            self.loss1 = self.loss1.cuda()
    ...
    def __call__(self, frac_epoch, inp, target):
        ...
        record = {}
        # print(self.loss1(output, target))
        # print(totalActivation(norm1))
        loss = self.loss1(output, target)
        record['loss'] = loss.item()
        record['top1'], record['top5'] = accuracy(output, target, topk=(1, 5))
        record['top1Count'] = record['top1']
        record['top5Count'] = record['top5']
        record['top1'] /= len(output)
        record['top5'] /= len(output)
        # print('top1 ' + str(record['top1']) + ' top5 ' + str(record['top5'])
        #       + ' loss1 ' + str(self.loss1(output, target).item()) + ' loss2 ' + str(totalActivation(norm1)) + ' totalLoss ' + str(record['loss']))
...
class ImageNetVal(object):
    ...
    def __init__(self, model):
        ...
        self.model = model
        self.data_loader = self.data()
        self.loss1 = nn.CrossEntropyLoss(size_average=False)
        # self.loss2 = totalActivation()
        if FLAGS.ngpus > 0:
            self.loss1 = self.loss1.cuda()
            # self.loss2 = self.loss2.cuda()
    ...
    def __call__(self):
        ...
        with torch.no_grad():
            for (inp, target) in tqdm.tqdm(self.data_loader, desc=self.name):
                ...
                record['loss'] += (self.loss1(output, target).item())
                p1, p5 = accuracy(output, target, topk=(1, 5))
                record['top1'] += p1
                record['top5'] += p5
        # for key in record:
        #     record[key] /= len(self.data_loader.dataset.samples)
        record['top1']/= len(self.data_loader.dataset.samples)
        record['top5']/= len(self.data_loader.dataset.samples)
        record['loss']= record['loss']/len(self.data_loader.dataset.samples)
        print('top1 ' + str(record['top1']) + 'top5 ' + str(record['top5']))
...
if __name__ == '__main__':
    for whichModel in [1]:
        ...
        if whichModel == 1:
            FLAGS.model = 'ST'
            FLAGS.numCate = 16
            FLAGS.data_path = '/home/tonglab/Datasets/imagenet'+str(FLAGS.numCate)
            FLAGS.output_path = './cornet/cornet_st/cornet_st_1000cate/'
            os.makedirs(FLAGS.output_path, exist_ok=True)
            train(restore_path=FLAGS.output_path + 'latest_checkpoint.pth.tar')
        # train()