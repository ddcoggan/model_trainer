...
nest layers into sequential ordered dicts for simplicity
superficial changes to naming conventions
"""
...
class LGN(nn.Module):
    def __init__(self, channels, kernel_size=11, stride=2):
        ...
        self.conv = nn.Conv2d(
            ...
            bias=False)
        self.norm = nn.BatchNorm2d(num_features=channels[1])
...
class FLaBBlock(nn.Module):
    ...
    :param project_back: bool, whether to include backward connection
    """

    def __init__(self, channels, norm_groups, kernel_size=3, stride=2,
                 cardinality=32, lat_pad=1, back_pad=3, project_back=True,
                 se_ratio=0.25):
        super().__init__()
        ...
        self.skip = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(channels[1], channels[2], kernel_size=1,
                               stride=stride, bias=False)),
            ('norm', nn.GroupNorm(norm_groups[2], channels[2])),
        ]))

        # forward projection (resnext-style bottleneck with squeeze-excitation)
        self.fwd = nn.Sequential(OrderedDict([
            ...
            ('b', nn.Sequential(OrderedDict([
                ('conv',
                 nn.Conv2d(channels[2], channels[2], kernel_size=kernel_size,
                           stride=stride,
                           padding=kernel_size // 2, groups=cardinality,
                           bias=False)),
                ('norm', nn.GroupNorm(norm_groups[2], channels[2])),
                ('relu', nn.LeakyReLU()),
            ]))),
            ('se', SqueezeExcitation(channels[2],
                                     int(round(channels[2] * se_ratio)),
                                     nn.LeakyReLU)),
            ('c', nn.Sequential(OrderedDict([
                ('conv', nn.Conv2d(channels[2], channels[2], kernel_size=1,
                                   bias=False)),
                ('norm', nn.GroupNorm(norm_groups[2], channels[2])),
            ]))),
        ]))
        ...
        # lateral projection
        self.lat = nn.Sequential(OrderedDict([
            ('conv', nn.ConvTranspose2d(channels[2], channels[1], kernel_size=kernel_size,
                stride=stride, padding=1, output_padding=lat_pad, bias=False)),
            ('norm', nn.GroupNorm(norm_groups[1], channels[1])),
            ('nonlin', nn.LeakyReLU()),
        ]))

        # backward projection
        if project_back:
            self.bck = nn.Sequential(OrderedDict([
                ('conv', nn.ConvTranspose2d(channels[2], channels[0], kernel_size=5, stride=4,
                    padding=2, output_padding=back_pad, bias=False)),
                ('norm', nn.GroupNorm(norm_groups[0], channels[0])),
                ('nonlin', nn.LeakyReLU()),
            ]))
        ...
        self.f_output = Identity()
        self.l_output = Identity()
        self.b_output = Identity()
    ...
    def forward(self, inputs):
        ...
        f = self.fwd(inputs)
        l = self.lat(f)
        b = self.bck(f) if self.project_back else 0
        f = f + skip
        f = self.nonlin(f)
...
class FLaB(nn.Module):
    ...
    def __init__(self, cycles=8, return_cycles='all'):
        super().__init__()
        ...
        self.V1 = FLaBBlock(channels=chn[0:3], cardinality=crd[0],
                            norm_groups=gnm[0:3], project_back=False)
    ...
    def forward(self, inputs):
        ...
        for c in range(cycles):
            ...
            for block in self.recurrent_blocks:
                ...
                flb = f + l + b  # sum inputs
                outputs = getattr(self, block)(flb)  # forward pass
                states[block] = {k: v for k, v in zip('flb', outputs)}  # store

            if self.return_cycles == 'all':
                decoder_outputs.append(self.decoder(states['IT']['f']))

        if self.return_cycles == 'all':
            return torch.stack(decoder_outputs, dim=1)  # dim 1 for dataparallel
        else:
            return self.decoder(states['IT']['f'])
