...
v17 performed well, but is much larger than blt with no performance advantages
v18 has an additional v3 block to see if we can approach cornet-s performance 
on imagenet. To counter the additional feature map width reduction, maxpool 
is removed from LGN.
"""
...
class CogDecoder(nn.Module):
    ...
    """
    Decoder for CogNet, containing adapative avgpool followed by a series of
    ...
    def __init__(self, in_channels, out_features=1000, head_depth=1,
                 head_features=None, head_width=3):
        super().__init__()
...
class CogNet(nn.Module):
    ...
    def __init__(self, cycles=5):
        ...
        self.cycles = cycles
        chn = [3, 64, 128, 256, 512, 1024, 2048]

        self.LGN = LGN(chn[0:2])
        self.V1 = CogBlock(channels=chn[0:3], back=False)
        self.V2 = CogBlock(channels=chn[1:4])
        self.V3 = CogBlock(channels=chn[2:5])
        self.V4 = CogBlock(channels=chn[3:6])
        self.IT = CogBlock(channels=chn[4:7], lat_pad=0, back_pad=1)
        self.decoder = CogDecoder(in_channels=chn[-1], out_features=1000, 
                                  head_depth=1, head_width=3)
        self.blocks = ['LGN', 'V1', 'V2', 'V3', 'V4', 'IT', 'decoder']
