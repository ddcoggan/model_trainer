...
class CogV1(nn.Module):
    ...
    def __init__(self, channels, kernel_size=7, image_size=224, num_scales=5,
        ...
        self.convs = nn.ModuleList([nn.Conv2d(
                ic, oc, kernel_size=kernel_size, padding=kernel_size // 2,
                bias=False) for _ in scales])
    ...
    def forward(self, inputs):
        ...
        for transform, conv, w in zip(self.xforms, self.convs, self.windows):
            f_temp = transform(f)  # shrink image depending on eccentricity
            f_temp = conv(f_temp)  # apply convolution
            f_temp = self.resize(f_temp)  # grow back to original size
            if w > 0:
                # copy to output
                f_out[..., w:-w, w:-w] = f_temp[..., w:-w, w:-w]  
            else:
                f_out = f_temp
