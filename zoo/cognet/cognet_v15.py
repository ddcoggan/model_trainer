...
batchnorm in LGN
resnext style convolution, i.e., groups
WARNING: this mixes resnet and resnext style bottlenecks, and so is probably 
not optimal.
"""
...
class CogNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        video = len(inp.shape) == 5
        if video:
            print(f'Video input detected, changing num cycles to fit input')
            cycles = inp.shape[0]
        else:
            cycles = self.cycles

        # initialize recurrent block states
        blocks = self.blocks[1:-1]
        ...
        for c in range(cycles):
            ...
            # if video, input next frame to LGN
            if video:
                from_lgn = LGN(inp[c])
            # if image, input to LGN only once
            elif c == 0:
                from_lgn = self.LGN(inp)
            ...
            for blk in blocks:
                ...
                b = 0 if blk == 'IT' or c == 0 else states[nxt_blk]['b']
