...
class CogV1(nn.Module):
    ...
    def __init__(self, channels, kernel_size=7, image_size=224, num_scales=5,
        ...
        self.windows = torch.linspace(0, image_size / 2, num_scales+1,
                                      dtype=torch.int)[:-1]
        for cyc in range(4):
            setattr(self, f'norm_{cyc}', nn.BatchNorm2d(oc))
        self.nonlin = nn.ReLU()
        ...
        for cyc in range(4):
            setattr(self, f'norm_lat_{cyc}', nn.BatchNorm2d(ic))
        self.nonlin_lat = nn.ReLU()


    def forward(self, inputs, cyc):
        ...
        f = getattr(self, f'norm_{cyc}')(f_out)

        # outgoing lateral connection
        l = self.conv_lat(f)
        l = getattr(self, f'norm_lat_{cyc}')(l)
        l = self.nonlin_lat(l)
...
class CogBlock(nn.Module):
    ...
    def __init__(self, channels, kernel_size=3, stride=1, scale=4):
        ...
        for cyc in range(4):
            setattr(self, f'norm_skip_{cyc}', nn.BatchNorm2d(oc))

        self.conv1 = nn.Conv2d(oc, sc, kernel_size=1, bias=False)
        for cyc in range(4):
            setattr(self, f'norm1_{cyc}', nn.BatchNorm2d(sc))
        ...
        self.conv2 = nn.Conv2d(sc, sc,
                               ...
                               padding=kernel_size//2, bias=False)
        for cyc in range(4):
            setattr(self, f'norm2_{cyc}', nn.BatchNorm2d(sc))
        self.nonlin2 = nn.ReLU()

        self.conv3 = nn.Conv2d(sc, oc, kernel_size=1, bias=False)
        for cyc in range(4):
            setattr(self, f'norm3_{cyc}', nn.BatchNorm2d(oc))
        ...
        self.conv_back = nn.Conv2d(oc, pc, kernel_size=3, padding=1,
                                             bias=False)
        for cyc in range(4):
            setattr(self, f'norm_back_{cyc}', nn.BatchNorm2d(pc))
    ...
    def forward(self, inputs, cyc):
        ...
        skip = getattr(self, f'norm_skip_{cyc}')(skip)

        # expansion convolution
        f = self.conv1(f)
        f = getattr(self, f'norm1_{cyc}')(f)
        f = self.nonlin1(f)

        # middle convolution
        f = self.conv2(f)
        f = getattr(self, f'norm2_{cyc}')(f)
        f = self.nonlin2(f)

        # contraction convolution
        f = self.conv3(f)
        f = getattr(self, f'norm3_{cyc}')(f)
        ...
        if self.prev_channels != self.in_channels:
            b = self.conv_back(b)
            b = getattr(self, f'norm_back_{cyc}')(b)
...
class CogNet(nn.Module):
    ...
    def __init__(self, cycles=4, return_states=False, return_blocks=None):
        super().__init__()

        self.cycles = cycles
        self.return_states = return_states
        if self.return_states:
            self.return_blocks = return_blocks if return_blocks else [
                'V1', 'V2', 'V4', 'IT']
        else:
            self.return_blocks = None
    ...
    def forward(self, inp):
        ...
        states = {block: {f'cyc{cycle:02}': {
            'f': 0, 'l': None, 'b': None, 'i': None
        } for cycle in np.arange(stored_cycles)} for block in blocks} 
        if self.return_states:
            states['output'] = {
                f'cyc{cycle:02}': None for cycle in np.arange(stored_cycles)}
        ...
        for c in range(cycles):
            ...
            for blk in blocks:
                ...
                f = inp_c if blk == 'V1' else states[prv_blk][cur_cyc]['f']

                # get lateral inputs from current block, previous cycle
                l = states[blk][prv_cyc]['l'] if c > 0 else None

                # get feedback inputs from next block, previous cycle
                b = states[nxt_blk][prv_cyc]['b'] if blk != 'IT' and c != 0 \
                    else None

                # get maxpool indices from previous block
                i = states[prv_blk][cur_cyc]['i'] if blk != 'V1' else 0

                # forward pass
                inputs = [f, l, b, i]
                outputs = getattr(self, blk)(inputs, c)

                # store outputs
                states[blk][cur_cyc] = {key: item for key, item in zip(
                    ['f', 'l', 'b', 'i'], outputs)}
            
            # store decoder states for each cycle
            if self.return_states:
                states['output'][cur_cyc] = self.decoder(f)

        if self.return_states:
            states = {
                k: v for k, v in states.items() if k in self.return_blocks}
            return states
        else:
            del states
