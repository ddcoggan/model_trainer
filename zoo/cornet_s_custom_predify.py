...
class cornet_s_custom_predifySeparateHP(PNetSeparateHP):
    def __init__(self, backbone, build_graph=False, random_init=False, ff_multiplier=(0.2,0.4,0.4,0.5,0.6), fb_multiplier=(0.05,0.01,0.01,0.01,0.0), er_multiplier=(0.01,0.01,0.01,0.01,0.01)):
        super().__init__(backbone, 5, build_graph, random_init, ff_multiplier, fb_multiplier, er_multiplier)

        # PCoder number 1
        pmodule = Sequential(Upsample(scale_factor=(4.0, 4.0), mode='bilinear', align_corners=False),ConvTranspose2d(64, 3, kernel_size=3, stride=1, padding=1))
        ...
        def fw_hook1(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.V1.cycle0.register_forward_hook(fw_hook1)

        # PCoder number 2
        pmodule = Sequential(Upsample(scale_factor=(2.0, 2.0), mode='bilinear', align_corners=False),ConvTranspose2d(64, 64, kernel_size=3, stride=1, padding=1))
        ...
        def fw_hook2(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.V1.CORblock.register_forward_hook(fw_hook2)
        ...
        def fw_hook3(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.V2.register_forward_hook(fw_hook3)
        ...
        def fw_hook4(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.V4.register_forward_hook(fw_hook4)

        # PCoder number 5
        pmodule = Sequential(Upsample(scale_factor=(1.75, 1.75), mode='bilinear', align_corners=False),ConvTranspose2d(512, 256, kernel_size=3, stride=1, padding=1))
        ...
        def fw_hook5(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.IT.register_forward_hook(fw_hook5)
