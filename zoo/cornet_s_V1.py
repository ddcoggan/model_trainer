...
class COGV1(nn.Module):
    ...
    def __init__(self, kernel_size=7, image_size=224, num_scales=5,
        ...
        scales = linspace(max_scale, min_scale, num_scales)
        self.xforms = nn.ModuleList([transforms.Resize(
            int(image_size // scale), antialias=True) for scale in scales])
        self.conv = nn.Conv2d(3, 128, kernel_size=kernel_size, padding=kernel_size // 2, bias=False)
        self.resize = transforms.Resize(image_size, antialias=True)
        self.windows = linspace(0, image_size / 2, num_scales + 1, dtype=int)[:-1]
